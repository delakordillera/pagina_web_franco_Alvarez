document.addEventListener("DOMContentLoaded", () => {
  ...
  if (whatsappBtn) {
    ...
    tooltip.style.cssText =
      "position:fixed;background:#fff;color:#20281f;padding:10px 16px;border-radius:12px;font-size:0.85rem;font-family:'Inter',sans-serif;box-shadow:0 8px 28px rgba(32,40,31,0.12);z-index:1060;max-width:220px;line-height:1.4;pointer-events:none;opacity:0;transition:opacity 0.3s ease,transform 0.3s ease;border:1px solid rgba(232,226,212,0.6);";

    function positionTooltip() {
      const rect = whatsappBtn.getBoundingClientRect();
      const gap = 10;
      tooltip.style.bottom = (window.innerHeight - rect.top - rect.height / 2 + tooltip.offsetHeight / 2) + "px";
